...
function isFav(name) {
  ...
  return new Set(favorites).has(name)
}

function hasFavChildren(name) {
  let outcome = false
  const children = getGroups(window.fontsList)[name] || []
  children.forEach((child) => {
    if (isFav(child.alias)) {
      outcome = true
    }
  })
  return outcome
...
function renderItem(data, nChildren=0) {
...
function renderSelectList () {
  ...
  fonts.sort((a, b) => {
    if ((isFav(a.alias) && !isFav(b.alias)) || (hasFavChildren(a.alias) && !hasFavChildren(b.alias))) {
      return -1
    }
    if ((!isFav(a.alias) && isFav(b.alias)) || (!hasFavChildren(a.alias) && hasFavChildren(b.alias))) {
      return 1
  ...
  fonts.forEach((v) => {
    ...
    root.appendChild(renderItem(v, children.length))

    children.forEach((c) => {
      root.appendChild(renderItem(c))
    })
  })
