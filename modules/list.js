export class List {
  ...
  constructor (all_data, filtered) {
    this.allowGrouping = Object.keys(all_data).length === filtered.length
  ...
  isFav(name) {
    return new Set(this.favorites).has(name)
  }

  hasFavChildren(name) {
    let outcome = false
    const children = this.getGroups(window.fontsList)[name] || []
    children.forEach((child) => {
      if (this.isFav(child.alias)) {
        outcome = true
      }
    })
    return outcome
  ...
  renderContent(data, chevron, heart) {
    return `
      ...
      ${data.website ? `<a class="website" href="${data.website}" rel="external"> <span>Website</span>${this.arrowIcon}</a>` : ''}
    `
  }
